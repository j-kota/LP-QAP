...
import MatchingLayer

class Aligned_Generator(object):
    ...
    def compute_example(self):
        ...
        # JK - changes begin right here
        # compute A right here and compute operators for it

        A = MatchingLayer.compute_alignment(W,W_noise)


        WW, x = self.compute_operators(A)       #W)
    ...
    def load_dataset(self):
        ...
        if os.path.exists(path):
            ...
            self.data_test = np.load(open(path, 'rb'),allow_pickle=True)   #JK mod
        else:
    ...
    def sample_batch(self, num_samples, is_training=True,
        ...
        # The trailing ',' is to match patterns in the training loop for pass to siamese gnn
        if cuda:
